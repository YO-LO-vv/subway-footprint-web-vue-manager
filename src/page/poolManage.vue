	...
	<div class="ordering">
			...
			<el-row class="searchInput">
				...
				<el-col :span="2">
					<el-select v-model="searchItem.selectInfo" placeholder="查询方式">
						<el-option v-for="item in searchItem.options" :key="item.value" :label="item.label"
							:value="item.value" />
					</el-select>
				</el-col>
				<el-col :span="2">
		...
		<div>
			<el-button type="primary" class="addAward" @click="handleAdd">
				新增奖品<el-icon class="el-icon--right">
					<Upload />
				</el-icon>
			</el-button>
		</div>
        ...
        " style="width: 100%;" class="awa-tab" empty-text="未查询到相关数据">
				...
				<el-table-column label="当前状态" min-width="180">
					<template #default="scope">
						<el-tag type="info" v-if="scope.row.status == '0'">已上架</el-tag>
				...
				<el-table-column label="操作" min-width="150">
					<template #default="scope">
						...
						<el-button size="small" @click="handleUpDown(scope.$index)" type="success"
							v-if="scope.row.status == 0">下架</el-button>
						<el-button size="small" @click="handleUpDown(scope.$index)" type="info" v-else>上架</el-button>
		...
		<div class="pagination">
			<el-pagination layout="prev, pager, next" :total="tableData.arr.length" @current-change="handleChange"
				:hide-on-single-page="true" background :page-size="pageSize" v-model:current-page="currentPage" />
		</div>
	</div>
	<!-- 编辑视图 -->
	<el-dialog v-model="dialogVisible" title="奖品详情" width="50%">
		<!-- 第一行 -->
		<el-row>
			<el-col :span="11">
				<el-form :model="formInline">
					<el-form-item label="奖品编号">
						<el-input v-model="formInline.now_award.aid" placeholder="{{formInline.now_award.aid}}"
							disabled />
					</el-form-item>
				</el-form>
			</el-col>
				...
				<el-form>
					<el-form-item label="奖品类型">
						<el-select v-model="formInline.now_award.variety"
							placeholder="{{formInline.now_award.variety}}">
			...
			<el-col :span="11">
				<el-form :model="formInline">
					<el-form-item label="商户编号">
						<el-input v-model="formInline.now_award.mid" placeholder="{{formInline.now_award.mid}}" disabled
							v-if="formInline.now_award.mid" />
			...
			<el-col :span="11">
				<el-form :model="formInline">
					<el-form-item label="提供日期">
						<el-input v-model="formInline.now_award.fromdate"
							placeholder="{{formInline.now_award.fromdate}}" disabled />
					</el-form-item>
				</el-form>
			</el-col>
			<el-col :span="2" />
			<el-col :span="11">
				<el-form :model="formInline">
					<el-form-item label="截止日期">
						<!-- 日期选择器 拿下 -->
						<el-date-picker v-model="formInline.now_award.todate" type="datetime"
							placeholder="{{formInline.now_award.todate}}" value-format="YYYY年MM月DD日hh:mm:ss" />
					</el-form-item>
				</el-form>
			</el-col>
		</el-row>
		...
		<div class="dialog-foot">
			<el-button type="primary" @click="editDown">确认</el-button>
		</div>
	</el-dialog>

	<!-- 新增视图 -->
	<el-dialog v-model="dialogVisible2" title="新增奖品" width="50%">
		<!-- 第一行 -->
		<el-row>
			<el-col :span="11">
				<el-form :model="formInline">
			...
			<el-col :span="11">
				<el-form :model="formInline">
					<el-form-item label="奖品数量">
						<el-input v-model="formInline.now_award.num" type="number" min="1"
		...
		<el-row>
			<el-col :span="11">
				<el-form :model="formInline">
					<el-form-item label="对应碳积分">
						<el-input v-model="formInline.now_award.credit" type="number" min="1"
							oninput="value=value.replace(/[^\d]/g,'')" />
					</el-form-item>
				</el-form>
			</el-col>
		...
		<el-row>
			...
			<el-col :span="11">
				<el-form :model="formInline">
					<el-form-item label="截止日期">
						<!-- 日期选择器 拿下 -->
						<el-date-picker v-model="formInline.now_award.todate" type="datetime" placeholder="选择截止日期"
		...
		<el-form :model="formInline">
			<el-form-item label="奖品内容">
				<el-input v-model="formInline.now_award.content" type="textarea" />
			</el-form-item>
		</el-form>
		<!-- 确认按钮 -->
		<div class="dialog-foot">
			<el-button type="primary" @click="addDown">确认</el-button>
		</div>
	</el-dialog>
</template>
	...
	export default {
		...
		setup() {
			...
			const handleUpDown = (index) => {
				formInline.which_one = index + pageSize.value * (currentPage.value - 1);
				tableData.arr[formInline.which_one].status =
					tableData.arr[formInline.which_one].status == 1 ? 0 : 1;
				upda();
				ElMessage({
					message: tableData.arr[formInline.which_one].status == 0 ?
						"上架成功" :
						"下架成功",
					type: "success",
				});
